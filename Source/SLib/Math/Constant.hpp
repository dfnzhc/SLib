...
constexpr Float kEpsilon = std::is_same_v<Float, f32> ? CastTo<Float>(kEpsilonF) : CastTo<Float>(kEpsilonD);

#define DEFINE_CONSTANT_FUNC(name)        \
    template<cFloatType F = f32>          \
    SLIB_FUNC constexpr F name() noexcept \
    {                                     \
        return CastTo<F>(k##name);        \
    }
...
SLIB_FUNC constexpr T MachineEpsilon() noexcept
{
    return numeric_limits<T>::epsilon();
}

template<cFloatType T>
SLIB_FUNC constexpr T Infinity() noexcept
{
    return numeric_limits<T>::infinity();
}

template<cArithmeticType T>
SLIB_FUNC constexpr T Zero() noexcept
{
    return T(0);
}

template<cFloatType T>
SLIB_FUNC constexpr T Half() noexcept
{
    return T(0.5);
}

template<cArithmeticType T>
SLIB_FUNC constexpr T One() noexcept
{
    return T(1);
}

template<cArithmeticType T>
SLIB_FUNC constexpr T Two() noexcept
{
    return T(2);
}

template<cArithmeticType T>
SLIB_FUNC constexpr Float Radians(T x) noexcept
{
    return static_cast<Float>(x) * static_cast<Float>(0.01745329251994329576923690768489);
}

template<cArithmeticType T>
SLIB_FUNC constexpr Float Degrees(T x) noexcept
{
    return static_cast<Float>(x) * static_cast<Float>(57.295779513082320876798154814105);
}

template<cArithmeticType T, i32 N>
SLIB_FUNC constexpr T Pow(T v) noexcept
{
    if constexpr (N == 0)
        return 1;
...
template<cArithmeticType T>
SLIB_FUNC constexpr T Pow2(T v) noexcept
{
    return Pow<T, 2>(v);
}
} // namespace slib